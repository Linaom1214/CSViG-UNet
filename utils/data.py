...
__all__ = ['SirstAugDataset', 'SirstDataset', 'MDFADataset', 'MergedDataset', 'IRSTDDataset', 'V3Dataset']
...
class V3Dataset(Data.Dataset):
    def __init__(self, base_dir='./data/V3', mode='train', base_size=512):
        assert mode in ['train', 'test']

        if mode == 'train':
            with open(osp.join(base_dir, 'train.txt'), 'r') as f:
                self.names = f.readlines()
        elif mode == 'test':
            with open(osp.join(base_dir, 'test.txt'), 'r') as f:
                self.names = f.readlines()
    ...
    def __getitem__(self, i):
        name = self.names[i]
        img_path = osp.join(self.base_dir, 'images', name)
        label_path = osp.join(self.base_dir, 'masks', name)
