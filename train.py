...
from utils.loss import SoftLoULoss, DiceLoss, WeightedLoss
from utils.lr_scheduler import *
from utils.metrics import SegmentationMetricTPFNFP
from utils.utils import Batch_Augmentation1, Batch_Augmentation2

def parse_args():
    #
    # Setting parameters
    #
    parser = ArgumentParser(description='Implement of CS-ViG-UNet')
    ...
    parser.add_argument('--weights', type=str, default=None, help='choose weights')
    #
    # Training parameters
    #
    ...
    parser.add_argument('--lr-scheduler', type=str, default='power', help='learning rate scheduler')
...
class Trainer(object):
    def __init__(self, args):
        ...
        if args.dataset == 'sirstaug':
            ...
            trainset = V3Dataset(base_dir='./data/V3', mode='train', base_size=args.base_size)
            valset = V3Dataset(base_dir='./data/V3', mode='test', base_size=args.base_size)
        elif args.dataset == 'ms':
            trainset = V3Dataset(base_dir='./data/MSISTD', mode='train', base_size=args.base_size)
            valset = V3Dataset(base_dir='./data/MSISTD', mode='test', base_size=args.base_size)
        else:
